...
\emph{module} with \verb|Import::scope_widened = true| --- wider than rustc.
Module-level \verb|use| (including inside inline \verb|mod| blocks) is
attributed to that module correctly. The flag exists so the limitation is never
silent.
...
\begin{description}[style=nextline,leftmargin=1em]
  ...
  \item[Cross-crate facade re-exports.] \verb|pub use| barrels across path
    dependencies are not followed; free functions behind a facade remain
    \verb|Unresolved| (measured on ripgrep).
\end{description}
...
\begin{enumerate}[leftmargin=1.5em]
  ...
  \item \textbf{Dev-dependencies as external roots.} Whether to list them so
    ...
    \verb|Unresolved|.
  \item \textbf{Facade re-exports across path deps.} Fix versus accept as a
    known limitation.
\end{enumerate}
