...
Methods and `impl` items never appear as `Function` nodes. Type definitions
and inherent-method analysis live in the sibling crate `horizon-types` (see
that crate’s README), not in this free-function JSON contract. Older maps that
still carry a `types` array or `receiver_type` field continue to load: serde
...
Module nodes are still not emitted; inline modules contribute path segments on
`Function.module_path` / `File.module_path` but do not appear as separate
containers. Type nodes are not part of this contract — see `horizon-types`.
