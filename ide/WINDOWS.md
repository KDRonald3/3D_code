...
| Tool | Notes |
|---|---|
| **Node.js 22.x** | Use the version in `ide\code-oss\.nvmrc` (currently 22.19.0). Node 24+ breaks native module builds - see Common native failures. Close/reopen PowerShell after install. |
...
`Bootstrap.ps1` / `Build.ps1` patch Code-OSS `preinstall.js` so the pinned vscode **1.105.x** tree accepts **VS 2026** (upstream that pin only listed 2019/2022). They also set `vs2026_install` / `vs2022_install` from `vswhere` when needed (VS 2026 installs under `...\18\...`, not `...\2026\...`).
...
| `The property 'Statement' cannot be found on this object` | npm 10.x `npm.ps1` is incompatible with `Set-StrictMode -Version Latest`. The scripts call `npm.cmd`; if you invoke npm yourself from a strict-mode script, do the same. |
