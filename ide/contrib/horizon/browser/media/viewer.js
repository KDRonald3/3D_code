(() => {
  ...
  /**
   * Horizon Map viewer — IDE webview build.
   * HTTP /api/* calls are replaced by window.HorizonBridge postMessage.
   * Inspection canvas / sidecar HTTP are owned by W3 — see README.md.
   */
  const Bridge = window.HorizonBridge || null;
  const IDE_MODE = !!(Bridge && Bridge.isVsCode);
  ...
  async function runRailFixtureTable() {
    const res = await fetch(new URL("rail_layout_cases.json", document.currentScript?.src || location.href));
    if (!res.ok) {
      return {
        ok: false,
        total: 0,
  ...
  function selectFile(id, { reveal = false, push = false, keepFn = false } = {}) {
    ...
    /* W3: host may reveal the file in classic/inspection editors. */
    if (Bridge && id) {
      const node = fileNodes.find((n) => n.id === id);
      Bridge.selectFile({ fileId: id, filePath: node?.path || null });
    }
  }
  ...
  function positionFnPreview(anchor) {
    ...
    // Float over the canvas, never over the layers tree: the tree is the user's
    // place-marker and covering it defeats the point of previewing in place.
    ...
    // Fit the gutter between the layers tree and the hovered row, so the peek
    // covers neither the tree nor the Functions list it belongs to. Re-measure:
    // constraining the width reflows the source and changes the height.
  ...
  async function loadFnPreview(file, fn) {
    ...
    const req = {
      ...
      byteStart: fn.byte_start ?? 0,
      byteEnd: fn.byte_end ?? 0,
      expectedHash: file.content_hash || "",
    };
    let result;
    if (IDE_MODE && Bridge) {
      const body = await Bridge.requestSource(req);
      result =
        body && Array.isArray(body.tokens)
          ? { tokens: body.tokens }
          : { error: (body && (body.message || body.error)) || "unavailable" };
    } else {
      const params = new URLSearchParams({
        path: req.path,
        byte_start: String(req.byteStart),
        byte_end: String(req.byteEnd),
  ...
  async function fetchSourceInto(host, file, fn) {
    ...
    // IDE: prefer host Inspection canvas (W3). Still try sourceRequest for a
    // lightweight token preview inside the Inspector when the host fulfills it.
    if (IDE_MODE && Bridge) {
      setSourceHost(host, "loading", { meta });
      try {
        const body = await Bridge.requestSource({
          path: filePath,
          byteStart: start,
          byteEnd: end,
          expectedHash: hash,
        ...
        if (SOURCE_MESSAGES[err]) {
          setSourceHost(host, err, { meta, message: body && body.message });
          return;
        }
        // No tokens — show Inspection-canvas hint instead of a hard error.
        setSourceHost(host, "error", {
          meta,
          message:
            (body && body.message) ||
            "Source opens in the Inspection canvas (host). W3 wires rust-analyzer there.",
        });
      } catch (e) {
        if (gen !== sourceFetchGen) return;
        setSourceHost(host, "error", {
          meta,
          message:
            "Inspection canvas owns source (W3). Inline preview unavailable: " +
            String(e.message || e),
        });
      }
      return;
  ...
  /**
   * Apply a host analyseResult message (progress or terminal).
   * When status is `done`, expects `map` on the message or a follow-up mapData.
   */
  function applyAnalyseResult(body) {
    if (!body || typeof body !== "object") return;
    if (body.status === "running") {
      setAnalyseOverlay(true, body.path || undefined);
    ...
    if (body.status === "done") {
      ...
      if (body.map && typeof body.map === "object") {
        loadMap(body.map, body.path || body.label || "analyse");
        if (els.analyseForm) els.analyseForm.hidden = true;
      }
      // else: host should send mapData next
      return;
    }
    // idle
    stopAnalyseTimers();
  ...
  if (els.analyseWorkspace) {
    els.analyseWorkspace.addEventListener("click", () => {
      runWorkspaceAnalyse();
    });
  }
  ...
  els.jsonInput.addEventListener("change", () => {
    ...
    reader.onload = async () => {
      try {
        ...
        // IDE: no sidecar POST from webview.
  ...
  async function boot() {
    ...
    // Wire host → webview messages (mapData, analyseResult, selectFunction, …).
    if (Bridge) {
      Bridge.onMessage((msg) => {
        switch (msg.type) {
          case "mapData":
            if (msg.map) {
              try {
                loadMap(msg.map, msg.label || "host");
                bootStatus = {
                  phase: "loaded",
                ...
                showImport(
                  `Failed to load map: ${err && err.message ? err.message : err}`
                );
              }
            } else {
              showImport(msg.error || null);
            }
            break;
          case "analyseResult":
            applyAnalyseResult(msg);
            break;
          case "selectFunction":
            if (msg.functionId) {
              selectFunction(msg.functionId, {
                reveal: true,
                push: false,
                notifyHost: false,
              });
          ...
          case "selectFile":
            if (msg.fileId) {
              selectFile(msg.fileId, { reveal: true, push: false });
            }
            break;
          case "workspaceInfo":
            applyWorkspaceInfo({
    ...
    if (Bridge && IDE_MODE) {
      ...
      // Otherwise show the import / analyse-workspace screen.
      setTimeout(() => {
        if (!currentMap && bootStatus.phase === "readyPosted") {
