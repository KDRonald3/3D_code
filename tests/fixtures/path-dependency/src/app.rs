...
pub fn run(values: &[f64], label: &str) -> String {
    ...
    let index: usize = get(3);
    let _ = index;

    describe(&n)
}

/// Declared HERE, not in lib.rs. The file lives at `src/app/helper.rs` and its
/// path is `crate::app::helper`.
mod helper;

/// Three ways to reach `text::upper` from this module without writing `crate::`.
pub fn without_crate_prefix(s: &str) -> String {
    // `super` is the crate root, since `app` was declared in lib.rs
    let a = super::text::upper(s);

    // the re-export in lib.rs, reached the same relative way
    let b = super::shout_upper(s);

    // through the submodule declared in this file
    let c = helper::tag(s);

    // and the submodule reaching back up two levels
    let d = helper::via_super(s);
